...
class Renderer():
    def __init__(self, image_size=512):
        ...
        t = torch.from_numpy(np.array([[0., 0., 3.]])).cuda().float()
...
def weights2colors(weights):
    ...
    colors = np.load('../../../../part_colors.npz')['colors'][None]
    argmax_weights = np.zeros_like(weights)
    argmax_weights[np.arange(weights.shape[0]), weights.argmax(1)] = 1.
    verts_colors = argmax_weights[:,:,None] * colors
...
if __name__ == '__main__':
    ...
    for mesh_file in mesh_files:
        ...
        mesh.vertices = v - center
        image = render_trimesh(mesh, mode='p')
        images.append(image)
    imageio.mimwrite(join(root, 'demo.mp4'), images, fps=5)